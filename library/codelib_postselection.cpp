           ...
           const mat & randeff,const mat & cust_grp)
{
  ...
  mat AA = zeros<mat>(pc,pc);
  mat M = zeros<mat>(D,pc);
  mat b1 = zeros<mat>(1,pc);
               ...
               int D, int type)
{
  ...
  if(type==1){
    ...
    for(int d=0;d<D;d++){
      int s = d*n;
      int e = n-1+d*n;
      mat bb = B(span(s,e),span(0,pc-1));//B(span(s,e),span(pc,2*pc-1));
      mat b = bb(a_1,a_2);
...
mat get_wmat_cpp(const NumericVector & y, const mat & evec, 
                 const mat & x_1, const mat & x_2,const mat & x_3,
                 const mat & z_1, const mat & z_2,const mat & z_3,  
                 const mat & G_1, const mat & G_2,const mat & G_3, 
                 const vec & Gamma_1,const vec & Gamma_2,
                 ...
                 int n, int D)
{
  ...
  mat X_G_1 = join_rows(x_1,G_1);//x_1;
  mat X_G_2 = join_rows(x_2,G_2);
  mat X_G_3 = join_rows(x_3,G_3);
  
  IntegerVector r_1 = rep_each(seq(0,n-1),m-1);
  IntegerVector r_2 = seq(0,pc_1+pc_2+pc_3-1);
  uvec a_1 = as<uvec>(r_1);
  uvec a_2 = as<uvec>(r_2);
  ...
  for(int i=0;i<n;i++){
    int s = i*(m-1);
    int e = m-2+i*(m-1);
    mat temp = X_G_1(span(s,e),span::all)*Gamma_1;
    Xb_1(i,span::all) = trans(temp);
    temp = X_G_2(span(s,e),span::all)*beta_2;
    Xb_2(i,span::all) = trans(temp);
    temp = X_G_3(span(s,e),span::all)*Gamma_3;
  ...
  for(int d=0;d<D;d++){
    ...
    int s = d*n;
    int e = n-1+d*n;
    mat bb = B(span(s,e),span(0,pc_1+pc_2+pc_3-1));
    mat b = bb(a_1,a_2);
    
    // login Indicator
    vec temp_1 = sum(z_1%b(span::all,span(0,pc_1-1)),1)+C_1(span::all,d);
    mat temp_2 = Xb_1 + trans(reshape(temp_1,m-1,n));
    mat f_0 = -log(1+exp(temp_2));
    uvec ii = find_nonfinite(f_0);
    f_0(ii) = -temp_2(ii);
    ...
    //Play Time
    temp_1 = tau*(sum(z_2%b(span::all,span(pc_1,pc_1+pc_2-1)),1)+C_2(span::all,d));
    ...
    // Purchase Indicator
    temp_1 = sum(z_3%b(span::all,span(pc_1+pc_2,pc_1+pc_2+pc_3-1)),1)+C_3(span::all,d);
